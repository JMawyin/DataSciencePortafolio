...
knitr::opts_chunk$set(warning = FALSE) #Disable Warnings Globally
library(magrittr)
```
...
### Loading Data
```{r}
train_loc <- "/Users/josemawyin/Library/Mobile Documents/com~apple~CloudDocs/Data Science Masters /622/622HW3/car_eval_train.csv"
eval_loc <- "/Users/josemawyin/Library/Mobile Documents/com~apple~CloudDocs/Data Science Masters /622/622HW3/car_eval_test.csv"

traindatafile<- train_loc
...
```{r}
dim(trdata); dim(tstdata)
```

```{r}
sapply(trdata, class); sapply(tstdata, class)
```
...
### GBM#2
```{r}
gbm_model2<-gbm(class~buying+maint+doors+persons+lug_boot+safety,data=car_eval,n.trees=5000,interaction.depth=nlev, shrinkage=0.001,bag.fraction=0.8,distribution="multinomial",
